...
const LowStockReportPage: React.FC = () => {
  ...
  const getTableData = () => {
    ...
    return reportData.products
      .filter((item: any) => (item.currentStock || 0) <= (item.reorderLevel || 0))
        ...
        {reportData && (
          <div className='report-summary-section'>
            <Row gutter={[16, 16]}>
              ...
              <Col xs={24} sm={12} md={6}>
                <Card className='summary-card'>
                  <div className='summary-title'>Critical Items</div>
                  <div className='summary-value' style={{ color: '#ff4d4f' }}>
                    {getTableData().filter(item => item.status === 'Critical').length}
                  </div>
                </Card>
              </Col>
              <Col xs={24} sm={12} md={6}>
                <Card className='summary-card'>
                  ...
                  <div className='summary-value' style={{ color: '#ff4d4f' }}>
                    {getTableData().filter(item => item.status === 'Out of Stock').length}
                  </div>
                </Card>
              </Col>
              <Col xs={24} sm={12} md={6}>
                <Card className='summary-card'>
                  <div className='summary-title'>Reorder Required</div>
                  <div className='summary-value' style={{ color: '#faad14' }}>
                    {getTableData().filter(item => item.status === 'Low').length}
                  </div>
                </Card>
              </Col>
            </Row>
