...
const updateCheckpointSchema = Joi.object({
  ...
  inspectorName: Joi.string().min(1).max(255).optional(),
...
const createComplianceReportSchema = Joi.object({
  ...
  certification: Joi.string().max(255).optional(),
  validityPeriod: Joi.string().max(100).optional(),
...
const updateComplianceReportSchema = Joi.object({
  reportType: Joi.string().valid(...Object.values(ComplianceType)).optional(),
  reportDate: Joi.date().optional(),
  auditorName: Joi.string().min(1).max(255).optional(),
  certification: Joi.string().max(255).optional(),
  validityPeriod: Joi.string().max(100).optional(),
  status: Joi.string().valid(...Object.values(ComplianceStatus)).optional(),
  findings: Joi.string().max(2000).optional(),
  recommendations: Joi.string().max(2000).optional(),
  documentUrl: Joi.string().uri().optional(),
});
