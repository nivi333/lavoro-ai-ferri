...
# STEP 1: CREATE USER
# =========================================
print_section "STEP 1: Creating Test User on Live Server"

TIMESTAMP=$(date +%s)
USER_EMAIL="livetest${TIMESTAMP}@lavoro.com"
...
COMPANY_RESPONSE=$(curl -s -X POST "$BASE_URL/companies" \
  ...
  -d "{
    \"name\": \"Live Test Company ${TIMESTAMP}\",
    \"slug\": \"live-test-${TIMESTAMP}\",
...
for i in {1..3}; do
    CUSTOMER_RESPONSE=$(curl -s -X POST "$BASE_URL/companies/$COMPANY_ID/customers" \
      ...
      -H "Authorization: Bearer $COMPANY_TOKEN" \
      -d "{
        \"name\": \"Live Test Customer $i\",
        \"customerType\": \"BUSINESS\",
        \"companyName\": \"Customer Business $i Ltd\",
        \"email\": \"livecustomer$i@test.com\",
...
for i in {1..3}; do
    SUPPLIER_RESPONSE=$(curl -s -X POST "$BASE_URL/companies/$COMPANY_ID/suppliers" \
      ...
      -H "Authorization: Bearer $COMPANY_TOKEN" \
      -d "{
        \"name\": \"Live Test Supplier $i\",
        \"supplierType\": \"MANUFACTURER\",
        \"email\": \"livesupplier$i@test.com\",
...
for i in {1..3}; do
    MACHINE_RESPONSE=$(curl -s -X POST "$BASE_URL/machines" \
      ...
      -H "Authorization: Bearer $COMPANY_TOKEN" \
      -d "{
        \"name\": \"Live Test Machine $i\",
        \"machineType\": \"Spinning Machine\",
        \"model\": \"Model-00$i\",
        \"manufacturer\": \"Test Manufacturer\",
        \"serialNumber\": \"SN-LIVE-000$i\",
...
for i in {1..2}; do
    QC_RESPONSE=$(curl -s -X POST "$BASE_URL/quality/checkpoints" \
      ...
      -H "Authorization: Bearer $COMPANY_TOKEN" \
      -d "{
        \"checkpointName\": \"Live Test Checkpoint $i\",
...
for i in {1..2}; do
    GARMENT_RESPONSE=$(curl -s -X POST "$BASE_URL/textile/garments" \
      ...
      -d "{
        ...
        \"cutDate\": \"2024-12-0$i\",
        \"operatorName\": \"Live Test Operator $i\",
        \"lineNumber\": \"LINE-0$i\",
        \"qualityPassed\": true,
        \"defectCount\": 0,
        \"isActive\": true
...
# FINAL SUMMARY
# =========================================
print_section "LIVE SERVER TEST SUMMARY"
...
if [ $FAILED -eq 0 ]; then
    echo -e "\n${GREEN}✓ ALL TESTS PASSED ON LIVE SERVER!${NC}"
    exit 0
else
    echo -e "\n${RED}✗ SOME TESTS FAILED ON LIVE SERVER${NC}"
    exit 1
fi
